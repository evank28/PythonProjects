# CITATION: The following block of code comes from Google Sheet's API's
# Quickstart.py, with some modifications to suit our needs
...
def isSimilar(a, b):  # Written by: EK
    similarity = SequenceMatcher(None, a,
                                 b).ratio()  # This funcion (SequenceMatcher) Came from the built in difflib library. https://d.python.org/2/library/difflib.html#sequencematcher-objects
    if similarity >= 0.55:
        return True
    else:
        return False
...
def timeSubtraction(inTime, minutesToSubtract):  # Written by: EK
    """takes a time in datetime.time() format and an integer for the number of
    minutes to subtract from the time"""
...
def callTime(HouseOpen, EventType):  # Written by: WZ
    """return(call_time,seniorCrew,juniorCrew)"""
    call_time = 0
    juniorCrew = 0
    seniorCrew = 0
    if EventType == "Lecture":
        call_time = timeSubtraction(getTime(HouseOpen), 30)
        juniorCrew = 3
        seniorCrew = 1
    if EventType == "Panel Discussion":
        call_time = timeSubtraction(getTime(HouseOpen), 90)
        juniorCrew = 3
        seniorCrew = 2
    if EventType == "Dance or Drama Performance":
        call_time = timeSubtraction(getTime(HouseOpen), 120)
        juniorCrew = 3
        seniorCrew = 3
...
def main(values):  # Written by: EK, Debugged by both WZ and EK
    ...
    # 2. Loop through the necessary rows and write the relevant data to a dictionary - new_events
    new_events = {}
    for event in values[newRowFinder('lastrow.txt'):len(
            values)]:  # Only updates rows that have not been updated previously. See newRowFinder() .
        new_events.setdefault(event[2], {'dateOfEvent': event[3],
    ...
    for event in new_events:
        ...
        eventMonth = findMonth(new_events[event]['dateOfEvent'])
        nextMonth = nxtMonth(new_events[event]['dateOfEvent'])

        for rows in values[months[eventMonth]:months[nextMonth]]:
            # print(rows[1], "compared with", findDay(new_events[event]['dateOfEvent']))
            if int(rows[1]) == findDay(new_events[event][
                                           'dateOfEvent']):  # Checks to see if an event on the day of the event exists - if yes, it knows the event row to insert
                rowToInsert = values.index(rows) + 1
                indextoInsert = values.index(rows)
                break
            elif int(rows[1]) > findDay(new_events[event][
                                            'dateOfEvent']):  # Checks to see if an event after the day of the event exists, setting the index to insert as 1 row above
        ...
        if not isSimilar(values[indextoInsert][2],
                         event):  # Checks to see if an event with the same name exists on that date
        ...
        ct = callTime(new_events[event]['houseOpen'],
                      new_events[event]['eventType'])
        batch_update_values_request_body = {
            ...
            'data': [
                {"range": str(rangeName) + '!B' + str(rowToInsert),
                 "majorDimension": "ROWS",
                 "values": [[findDay(new_events[event]['dateOfEvent'])]]},
                {"range": str(rangeName) + '!C' + str(rowToInsert),
                 "values": [[event]]},
                {"range": str(rangeName) + '!D' + str(rowToInsert), "values": [[
                    "Event Coordinator: " + str(
                        new_events[
                            event][
                            'eventCoordinator']) + "\n" + "Staff Supervisor: " + str(
                        new_events[
                            event][
                            'staffSupervisor'])]]},
                {"range": str(rangeName) + '!E' + str(rowToInsert),
                 "values": [[ct[1]]]},
                {"range": str(rangeName) + '!G' + str(rowToInsert),
                 "values": [[ct[2]]]},
                {"range": str(rangeName) + '!J' + str(rowToInsert),
                 "values": [[formatTime(ct[0])]]},
                {"range": str(rangeName) + '!K' + str(rowToInsert),
                 "values": [["Type: " + str(new_events[event]['eventType'])
                             + "\n Event Starts: " + str(
                     new_events[event]['eventStart'])
                             + "\n House Opens: " + str(
                     new_events[event]['houseOpen'])
                             + "\n Curtains: " + str(
                     new_events[event]['curtains'])
                             + "\n Podium: " + str(new_events[event]['podium'])
                             + "\n Audience Questions: " + str(
                     new_events[event]['audienceQs'])
                             + "\n Lights On: " + str(
                     new_events[event]['Lighting'])
                             + "\n Additional AV Media: " + str(
                     new_events[event]['AvMedia'])
                             + "\n Additional Furniture: " + str(
                     new_events[event]['additionalFurniture'])
                             ]]}
            ],
...
if __name__ == '__main__':  # Written by: EK
    ...
    choice = eg.msgbox(
        msg='Credentials are incorrect - cannot run. Please Run the program again, after installing credentials.',
        title='Incorrect Credentials - Try Again.', ok_button='Exit',
        image=None, root=None)
    if choice == 1:
        exit()
# Written by: WZ
choice = eg.ynbox(msg="Spreadsheet Status:   " + checkStatus(values),
                  title='Event List Updater',
                  choices=['Update Spreadsheet Now', 'Exit'],
                  default_choice=default, cancel_choice='Cancel')
if (choice == 1):
    if __name__ == '__main__':
        main(values)
    option = eg.ynbox(
        "The Event List was updated succesfully. Do you wish to view it?",
        "Event List Updater - Update Complete",
        ("View Updated Event List", "Exit"))
    if option == 1:
        webbrowser.open(
            "https://docs.google.com/spreadsheets/d/1z2QzPf9Kc02roOwTJUYab4k2dwYu1n-nIbJ5yzWF3YE/edit?ts=58ecf7bf#gid=1034566956",
